...
import { ArrowDown, ArrowUp, ArrowUpDown, MessageSquare, ListChecks, ChevronLeft, ChevronRight } from 'lucide-react';
import { Skeleton } from '@/components/ui/skeleton';
import { Button } from '@/components/ui/button';
import { IncidentSeverityBadge, IncidentStatusBadge } from './incident-badges';
import type { IncidentFilters, IncidentListItem } from '@/types/incident';

const columnHelper = createColumnHelper<IncidentListItem>();
...
export function IncidentTable({
  ...
  const columns = React.useMemo(
    () => [
      columnHelper.accessor('code', {
        header: 'Code',
        cell: (info) => <span className="font-mono text-xs text-muted-foreground">{info.getValue().slice(0, 10)}</span>,
      }),
      ...
      columnHelper.display({
        id: 'people',
        header: 'Reporter / Assignee',
        cell: (info) => {
          const row = info.row.original;
          return (
            <div className="text-sm">
              <p>{row.reporter.firstName} {row.reporter.lastName}</p>
              {row.assignee && (
                <p className="text-xs text-muted-foreground">→ {row.assignee.firstName} {row.assignee.lastName}</p>
              )}
            </div>
          );
        },
      ...
      columnHelper.display({
        ...
        cell: (info) => (
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            {info.row.original._count.capas > 0 && (
              <span className="flex items-center gap-1"><ListChecks className="h-3 w-3" /> {info.row.original._count.capas}</span>
            )}
            {info.row.original._count.comments > 0 && (
              <span className="flex items-center gap-1"><MessageSquare className="h-3 w-3" /> {info.row.original._count.comments}</span>
            )}
          </div>
        ),
      }),
  ...
  const table = useReactTable({
    ...
    onSortingChange: (updater) => {
      ...
      onFiltersChange({
        ...filters,
        sortBy: (first?.id as IncidentFilters['sortBy']) ?? 'createdAt',
